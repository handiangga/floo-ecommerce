...
export default function NewProductPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (invalidVariant) {
      ...
      // Empty optional numeric inputs must not be sent as an empty string.
      // PostgreSQL rejects "" for BIGINT columns such as subcategory_id.
      const subcategoryId = String(productForm.get("subcategory_id") ?? "").trim();
      if (subcategoryId) productForm.set("subcategory_id", subcategoryId);
      else productForm.delete("subcategory_id");

      const weight = String(productForm.get("weight") ?? "").trim();
      productForm.set("weight", weight || "0");
    ...
    try {
      ...
      window.sessionStorage.setItem(
        ...
        "Produk dan semua variasi berhasil ditambahkan.",
      );
