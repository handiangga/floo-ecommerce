...
class HomepageCraftsmanshipService {
  ...
  async update(payload, files = []) {
    ...
    const existingImages = current?.images?.length ? [...current.images] : [...defaults.images];
    ...
    const gallery = [0, 1, 2, 3].map((index) => ({
      title: String(payload[`gallery_${index}_title`] || "").trim(),
      description: String(payload[`gallery_${index}_description`] || "").trim(),
    }));
